...
for i = 1:length(AoA_2)
    %for j = 1:length(M_inf)
        ...
        Cp_star(j,i) = (2/(GAMMA*M_inf1(j).^2))*(((2+(GAMMA-1)*M_inf1(j).^2)/(1+GAMMA))^(GAMMA/(GAMMA-1))-1);
        Cp_Laitone(j,i) = Cp_min/(sqrt(1-M_inf1(j)^2)+(Cp_min/2)*(M_inf1(j)^2/sqrt(1-M_inf1(j)^2))*(1+(GAMMA-1)*M_inf1(j)^2/2));
        Mcritic_eq = Cp_star-Cp_Laitone == 0;
        Mcritic_sol(i) = solve(Mcritic_eq, M_inf1);
       % error(j,i) = abs(Cp_Laitone(j,i)-Cp_star(j,i))/abs(Cp_star(j,i));
        %if error(j,i)<0.01;
         %   a=1
...
M_inf_3 = linspace(0.01, 0.35, 100);
% Preallocating
