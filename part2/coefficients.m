function [coef] = coefficients(rho,y,y_h,x_c,y_c,z_c,x_c_h,y_c_h,z_c_h,c,c_h,Q_inf,AoA, AoA_t,N_1,N_2,Cl_0,Cl_alpha,gamma, b_wing,b_tail, S_wing, S_tail,theta,Cd_v0,Cd_v_Cl,M_chord_ala,M_chord_tail, Cm_delta,Cm_025,delta_l,delta_r,delta_t)
...
for i = 1:N_1
    ...
    coef.Cd_i_wing(i,1) = -2*((gamma(i,1)*coef.alpha_i_wing(i,1))/(norm(Q_inf)*c(i,1)));
    Cd_i_wing2(i,1) = -2*((gamma(i,1)*(y(1,i+1)-y(1,i))*coef.alpha_i_wing(i,1))/(norm(Q_inf)*S_wing));
    ...
    coef.Cd_wing(i,1) = Cd_i_wing2(i,1)+coef.Cd_v_wing2(i,1);
...
CM_y = sum(Cm_y);
CM_z = sum(Cm_z);
coef.Cm_0_ala = Cm_025*AoA + Cm_delta*delta_l +Cm_delta*delta_r + CM_x + CM_y + CM_z;
...
for i = N_1+1:N_1+N_2
    ...
    coef.Cd_i_tail(i-N_1,1) = -2*((gamma(i,1)*coef.alpha_i_tail(i-N_1,1))/(norm(Q_inf)*c_h(i-N_1,1)));
    Cd_i_tail2(i-N_1,1) = -2*((gamma(i,1)*(y_h(1,i+1-N_1)-y_h(1,i-N_1))*coef.alpha_i_tail(i-N_1,1))/(norm(Q_inf)*S_tail));
    ...
    Cm_y_h_roll(i-N_1,1) = (y_c_h(i-N_1,1)*gamma(i,1)*(y(1,i+1-N_1)-y(1,i-N_1)));
...
CM_y_h = sum(Cm_y_h);
CM_z_h = sum(Cm_z_h);
coef.Cm_0_tail = Cm_025*AoA_t + Cm_delta*delta_t + CM_x_h; %CM_y_h + CM_z_h
coef.Effi_tail = coef.CL_tail/coef.CD_tail;
coef.M_roll_tail = rho*norm(Q_inf)*sum(Cm_y_h_roll);
